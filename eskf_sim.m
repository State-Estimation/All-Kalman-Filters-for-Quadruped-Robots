...
p_ = transpose((ground_truth_data{1, 2:4}))-[0;0;0.026];   %初始位置
v_ = [0; 0; 0];
R_ = a_R;
ba_ = [1; 1; 1] * 3.552e-04;
bg_ = [1; 1; 1] * 1.655e-05;
...
for i = 1:num_steps
    ...
    if i == 1
        [X, theta] = ConstructState(R_, v_, p_, d0, d1, d2, d3, ba_, bg_);
        ...
        %将滤波结果存储在数组中
        p_history(:, i) = X_update(1:3, 5) + [0;0;0.025];
...
% %需要打印的数据
% outcome_eskf = transpose([time;p_history(:,:);q_history(:,:)]);
% % 将 outcome_only_imu 保存为 MAT 文件
% save('ESKF.mat', 'outcome_eskf');
...
function [X_pred, theta_pred, P_pred] = PredictState(X, theta, P, gyro_, acce_, dt, gravity, i, contact_0, contact_1, contact_2, contact_3, Qg, Qa, Qco, Qbg, Qba)
    ...
    [R_, v_, p_, d0, d1, d2, d3, ba_, bg_] = SeperateState(X, theta);
    ...
    bg_pred = bg_;
    ba_pred = ba_;
    Q = blkdiag(Qg, zeros(3), Qa, Qba, Qbg, (Qco+(1e4*eye(3).*(1-contact_0(i)))), (Qco+(1e4*eye(3).*(1-contact_1(i)))), (Qco+(1e4*eye(3).*(1-contact_2(i)))), (Qco+(1e4*eye(3).*(1-contact_3(i)))));
    F11 = expm(-skew((gyro_(1:3, i) - bg_) * dt));
    F15 = -eye(3)*dt;
    F31 = -R_pred * skew(acce_(1:3, i) - ba_) * dt;
    F34 = -R_pred * dt;
    %     theta     p         v         ba         bg       d0        d1
    F = [ F11,    zeros(3), zeros(3), zeros(3),   F15,    zeros(3), zeros(3), zeros(3), zeros(3);
        zeros(3), eye(3),  eye(3)*dt, zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3);
          F31   , zeros(3), eye(3),     F34,    zeros(3), zeros(3), zeros(3), zeros(3), zeros(3);
        zeros(3), zeros(3), zeros(3), eye(3),   zeros(3), zeros(3), zeros(3), zeros(3), zeros(3);
        zeros(3), zeros(3), zeros(3), zeros(3), eye(3),   zeros(3), zeros(3), zeros(3), zeros(3);
        zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), eye(3),   zeros(3), zeros(3), zeros(3);
        zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), eye(3) , zeros(3), zeros(3);
        zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3) , eye(3), zeros(3);
        zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3) , zeros(3), eye(3);];
%    P_pred =  Ak * P * Ak' + Qk;
    P_pred =  F * P * F' + Q;
    [X_pred, theta_pred] =  ConstructState(R_pred, v_pred, p_pred, d0, d1, d2, d3, ba_pred, bg_pred);

end


%状态的更新过程
function [X_update, theta_update, P_update] = UpdateState(X_pred, theta_pred, P_pred, Y, H, N)

    S = H * P_pred *H' + N;
    K = (P_pred * H') / S;
    delta = K * Y;
    [X_update, theta_update] = PlusState(X_pred, theta_pred, delta);
    I = eye(size(P_pred));
...
function [X_update, theta_update, P_update] = UpdateState_forwardkinematics(X_pred, theta_pred, P_pred, contact_0, contact_1, contact_2, contact_3, d0_measure_pos, d1_measure_pos, d2_measure_pos, d3_measure_pos, i, Qe, Np)
    ...
    [R_pred, ~, p_pred, d0, d1, d2, d3, ~, ~] = SeperateState(X_pred, theta_pred);

    %四条腿同时与地面接触(参考结构)
%     if contact_0(i) == 1 && contact_1(i) == 1 && contact_2(i) == 1 && contact_3(i) == 1
%         d0_fx = R_pred' * ( d0 - p_pred ); 
%         d1_fx = R_pred' * ( d1 - p_pred );
%         d2_fx = R_pred' * ( d2 - p_pred );
%         d3_fx = R_pred' * ( d3 - p_pred );
%         
%         Y = [d0_measure_pos(1:3, i) - d0_fx;
%              d1_measure_pos(1:3, i) - d1_fx;
%              d2_measure_pos(1:3, i) - d2_fx;
%              d3_measure_pos(1:3, i) - d3_fx];     
%         H = [skew(d0_fx), -R_pred', zeros(3), zeros(3), zeros(3), R_pred', zeros(3), zeros(3), zeros(3);
%              skew(d1_fx), -R_pred', zeros(3), zeros(3), zeros(3), zeros(3), R_pred', zeros(3), zeros(3);
%              skew(d2_fx), -R_pred', zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), R_pred', zeros(3);
%              skew(d3_fx), -R_pred', zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), zeros(3), R_pred'];
...
%         [X_update, theta_update, P_update] = UpdateState(X_pred, theta_pred, P_pred, Y, H, N);
%     end

    % 获取当前激活的接触点索引（0-based）
    active_contacts = find([contact_0(i), contact_1(i), contact_2(i), contact_3(i)] == 1) - 1;

    % 预定义数据结构（根据实际变量名修改）
    contact_d = {d0, d1, d2, d3};
    contact_measure = {d0_measure_pos, d1_measure_pos, d2_measure_pos, d3_measure_pos};
    ...
    % 动态构建矩阵
    for k = active_contacts
        % 计算当前接触点相关量
        d_fx = R_pred' * (contact_d{k+1} - p_pred);
        measure_pos = contact_measure{k+1}(1:3, i);

        % 构建Y
        Y = [Y; measure_pos - d_fx];

        % 构建H行块
        H_row = cell(1,9);
        H_row(1:5) = {skew(d_fx), -R_pred', zeros(3), zeros(3), zeros(3)};
        H_row(6:9) = arrayfun(@(x) zeros(3), 1:4, 'UniformOutput', false);
        H_row{6+k} = R_pred';  % 根据接触点位置设置R_pred

        % 合并行块
        H_blocks = [H_blocks; cell2mat(H_row)];

        % 构建N块
        N_blocks = blkdiag(N_blocks, R_pred*Qe*R_pred' + Np);
    end

    % 执行状态更新
    if ~isempty(Y)
        [X_update, theta_update, P_update] = UpdateState(X_pred, theta_pred, P_pred, Y, H_blocks, N_blocks);
    else
        % 无接触时的处理
        X_update = X_pred;
        theta_update = theta_pred;
        P_update = P_pred;
    end  
...
%对状态进行分离，提取其中的各个状态变量
function [R_, v_, p_, d0, d1, d2, d3, ba_, bg_] = SeperateState(X, theta)
...
%利用状态变量构建状态
function [X, theta] = ConstructState(R_, v_, p_, d0, d1, d2, d3, ba_, bg_)
...
%更新x+dx状态
function [X_update, theta_update] = PlusState(X_pred, theta_pred, delta)
    dR = exp(delta(1:3));  
    dp = delta(4:6);  
    dv = delta(7:9);
    dtheta = delta(10:15); 
    ...
    d_d2 = delta(22:24);
    d_d3 = delta(25:27);
    [R_, v_, p_, d0, d1, d2, d3, ~, ~] = SeperateState(X_pred, theta_pred);
    X_update = [R_*dR, v_+dv, p_+dp, d0+d_d0, d1+d_d1, d2+d_d2, d3+d_d3];
